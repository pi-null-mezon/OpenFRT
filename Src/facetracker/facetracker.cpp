...
//---------------------------------------------------------------------------------
void FaceTracker::setFaceShapePredictor(dlib::shape_predictor *_dlibfaceshapepredictor)
{
    ...
    if(faceRect.area() > 0) {
        ...
        switch(m_method) {
            ...
            case FaceShape: {
                ...
                dlib::cv_image<unsigned char> _dlibfaceimg(faceImage);
                dlib::full_object_detection _shape = (*pt_dlibfaceshapepredictor)(_dlibfaceimg, dlib::rectangle(_dlibfaceimg.nc(),_dlibfaceimg.nr()));
                DLIB_CASSERT (_shape.num_parts() == 68 || _shape.num_parts() == 5, "\n\t Invalid inputs were given to this function. " << "\n\t d.num_parts():  " << _shape.num_parts());
                cv::Point2f _lefteyecenter;
                cv::Point2f _righteyecenter;
				if(_shape.num_parts() == 68) {
					for(size_t i = 0; i < 6; ++i) {
						_lefteyecenter += cv::Point2f(_shape.part(i+36).x(),_shape.part(i+36).y());
						_righteyecenter += cv::Point2f(_shape.part(i+42).x(),_shape.part(i+42).y());
					}
					_lefteyecenter /= 6;
					_righteyecenter /= 6;
				} else if(_shape.num_parts() == 5) {
                    for(size_t i = 0; i < 2; ++i) {
                        _righteyecenter += cv::Point2f(_shape.part(i).x(),_shape.part(i).y());
                        _lefteyecenter += cv::Point2f(_shape.part(i+2).x(),_shape.part(i+2).y());
                    }
                    _lefteyecenter /= 2;
                    _righteyecenter /= 2;
                }									
