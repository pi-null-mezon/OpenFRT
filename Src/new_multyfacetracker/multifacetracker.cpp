...
MultiFaceTracker::MultiFaceTracker(const cv::Ptr<FaceDetector> &_ptr, size_t maxfaces) :
    dPtr(_ptr),
    uuid(0)
{
    vtrackedfaces.resize(maxfaces);
}

std::vector<Mat> MultiFaceTracker::getResizedFaceImages(const Mat &_img, const Size &_size, int _averagelast)
{
    __enrollImage(_img);
...
std::vector<TrackedFace> MultiFaceTracker::getTrackedFaces() const
{
    return vtrackedfaces;
}

Mat MultiFaceTracker::__cropInsideFromCenterAndResize(const Mat &input, const Size &size)
{
...
void MultiFaceTracker::__enrollImage(const Mat &_img)
{
        ...
        for(size_t j = 0; j < vrects.size(); ++j) {
            if(alreadyused[j] == false) {
                if(_tf.getFramesTracked() > 0) {
                    ...
                    if((_lastrect & vrects[j]).area() > ((_lastrect.area() + vrects[j].area()) / 5)) {
...
unsigned long MultiFaceTracker::__nextUUID()
{
    return ++uuid;
}
...
void TrackedFace::setPosted2Srv(bool _value)
{
    posted2Srv = _value;
}
